...
const compressPngWithPngquant = async (inputBuffer: Buffer, quality: [number, number]) => {
  return new Promise<Buffer>((resolve, reject) => {
    const process = spawn(pngquant, [
    ...
    process.on('close', (code) => {
      if (code === 0) {
        ...
        reject(new Error('pngquant failed'));
      }
    });
  ...
  try {
    switch (format.split('/')[1]) {
      ...
      case 'jpg':
        ...
        break;
      case 'png': {
        const sharpBuffer = await image.png(selectedQuality.png).toBuffer();
        let minQ = 60, maxQ = 90;
        if (compressionLevel === 'minimal') { minQ = 80; maxQ = 100; }
        if (compressionLevel === 'maximum') { minQ = 40; maxQ = 80; }
        const pngquantBuffer = await compressPngWithPngquant(sharpBuffer, [minQ, maxQ]);
        if (pngquantBuffer.length && pngquantBuffer.length < buffer.length) {
          return pngquantBuffer;
        }
        return sharpBuffer;
      }
      case 'webp':
